...
public class PimApiClient(HttpClient http, ILogger<PimApiClient> logger)
{
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            return await http.GetFromJsonAsync<PagedResult<TaxonListItem>>(url, ct);
        }
        catch (Exception ex)
        {
    ...
    public async Task<TaxonDetail?> GetTaxonAsync(long id, CancellationToken ct = default)
    {
        try
        {
            return await http.GetFromJsonAsync<TaxonDetail>($"/api/v1/taxons/{id}", ct);
        }
        catch (Exception ex)
        {
        ...
        CancellationToken ct = default)
    {
        ...
        try
        {
            return await http.GetFromJsonAsync<List<SearchResult>>(
                $"/api/v1/search?q={Uri.EscapeDataString(q)}&limit={limit}", ct);
        }
        catch (Exception ex)
        {
        ...
        CancellationToken ct = default)
    {
        try
        {
            return await http.GetFromJsonAsync<List<CategorieArborescence>>(
                "/api/v1/categories/arborescence", ct);
        }
        catch (Exception ex)
        {
